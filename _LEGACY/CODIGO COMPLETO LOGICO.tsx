...
// Asegúrate de que estas rutas coincidan con tu estructura de carpetas actual
...
// ==========================================
// 2. DUAL GATEWAY (SELECTOR DE MODO)
// ==========================================
...
// ==========================================
// 3. ARCHITECT HUD (PANEL VENDEDOR)
// ==========================================
      ...
      <div className="bg-[#0a0a0a]/90 backdrop-blur-2xl border border-amber-500/30 rounded-t-3xl p-6 shadow-[0_0_50px_rgba(245,158,11,0.2)]">
        ...
        {step === 'SCAN' && (
          <div className="space-y-6">
            ...
            <button onClick={() => { sound?.playDeploy(); setStep('SKILL_TREE'); }} className="w-full py-4 bg-amber-600 hover:bg-amber-500 text-black font-bold text-sm tracking-widest rounded-xl shadow-lg transition-all flex items-center justify-center gap-2">
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-end border-b border-white/10 pb-4">
                <div>
                    ...
                    <p className="text-[10px] text-white/40 mt-1">Active nodos para atraer agentes.</p>
                </div>
            ...
            <div className="grid grid-cols-2 gap-3 max-h-[200px] overflow-y-auto custom-scrollbar">
                {SERVICES.map(srv => { 
                    ...
                    return (
                        <div key={srv.id} onClick={() => toggleService(srv)} className={`p-3 rounded-lg border cursor-pointer transition-all flex flex-col gap-2 ${active ? 'bg-white/10 border-amber-500/50' : 'bg-white/5 border-white/10 hover:border-white/30'}`}>
                            <div className="flex justify-between items-start">
                                <srv.icon size={16} className={active ? srv.color : 'text-white/30'} />
            ...
            <button onClick={() => { sound?.playDeploy(); setStep('INBOX'); }} className="w-full py-4 bg-white hover:bg-gray-200 text-black font-bold text-sm tracking-widest rounded-xl shadow-lg transition-all flex items-center justify-center gap-2">
          ...
          <div className="space-y-4">
             ...
             <div className="bg-gradient-to-br from-[#1a1a1a] to-black border border-amber-500/50 p-5 rounded-xl relative overflow-hidden group">
                ...
                <div className="flex gap-2">
                    <button className="flex-1 py-2 bg-amber-500 text-black text-xs font-bold rounded hover:scale-105 transition-transform">ACEPTAR ALIANZA</button>
        ...
        <div className="absolute top-80 left-8 z-[9000] flex flex-col gap-2 pointer-events-auto">
            <div className="bg-black/60 backdrop-blur-xl border border-white/10 p-1.5 rounded-xl shadow-xl flex flex-col gap-1.5">
                <button 
                    ...
                    className={`p-1.5 rounded-lg transition-all ${currentView.is3D ? 'bg-white/20 text-white' : 'text-white/50 hover:text-white'}`} 
                    title="3D"
                >
                    <Box size={16} />
                </button>
            ...
            <div className="bg-black/60 backdrop-blur-xl border border-white/10 p-1.5 rounded-xl shadow-xl flex flex-col gap-1.5">
                <button 
                    ...
                    className={`p-1.5 rounded-lg transition-all ${currentView.mode === 'dusk' ? 'bg-white/20 text-white' : 'text-white/50 hover:text-white'}`} 
                    title="Noche"
                >
                    <Moon size={16} />
                </button>
                <button 
                    ...
                    className={`p-1.5 rounded-lg transition-all ${currentView.mode === 'dawn' ? 'bg-white/20 text-white' : 'text-white/50 hover:text-white'}`} 
                    title="Día"
                >
                    <Sun size={16} />
                </button>
      ...
      <div className="bg-black/80 backdrop-blur-xl border border-white/10 p-4 rounded-xl w-full shadow-2xl transition-all" style={{borderColor: CORPORATE_BLUE + '60'}}>
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-[10px] text-white/60 font-mono cursor-pointer hover:text-white" onClick={cycleLang}>
              ...
              <span className="flex items-center gap-1" style={{color: CORPORATE_BLUE}}><Globe size={10} /></span>
          </div>
          <div className="flex justify-between text-[10px] text-white/60 font-mono cursor-pointer hover:text-white" onClick={toggleSound}>
              <span>{t.status.audio}</span>
              <span className={soundEnabled ? "text-emerald-400" : "text-gray-500"}>{soundEnabled ? 'ON' : 'OFF'}</span>
          </div>
        </div>
            ...
            <div className="mt-3 pt-3 border-t border-white/10 overflow-hidden animate-fade-in-up">
                {notifications.map((notif, i) => (
                    <div key={i} className="mb-3 last:mb-0 pb-2 last:pb-0 border-b last:border-b-0 border-white/5">
                        <div className="text-[9px] mb-1 font-mono uppercase w-full overflow-hidden" style={{color: CORPORATE_BLUE}}>
                            <span className="inline-block whitespace-nowrap animate-marquee">{notif.title}</span>
                        </div>
                        <div className="text-[10px] text-white/80 leading-tight w-full whitespace-normal">{notif.desc}</div>
                        {notif.action === 'CHAT' && (
                            <button onClick={() => onOpenChat()} className="mt-2 w-full py-1.5 bg-white/10 hover:bg-white/20 text-[9px] font-bold text-white rounded transition-colors flex items-center justify-center gap-1">
                                {t.commandPanel.contact} <ArrowRight size={10}/>
                            </button>
                        )}
                    </div>
                ))}
                <button onClick={clearNotifications} className="mt-3 py-1 text-[9px] text-white/50 hover:text-red-400 uppercase tracking-wider w-full text-center flex items-center justify-center gap-1 transition-colors">
                    <Trash2 size={10}/> {t.status.clear}
                </button>
            </div>
        )}
      </div>
...
export const CommandCenterPanel = ({ property, onClose, t, sound, onContactAgent }) => {
    ...
    // Componentes Gráficos Internos (SVG puro para rendimiento)
    const TrendChart = () => (
                ...
                <div className="bg-white/5 border border-white/10 p-6 rounded-2xl">
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="text-xs font-bold text-white/60 uppercase tracking-widest flex items-center gap-2">
                            <TrendingUp size={16}/> TENDENCIA MERCADO (5 AÑOS)
                        </h3>
    ...
    return (
        <div className="fixed inset-0 z-[55000] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 animate-fade-in">
            <div className="relative w-full max-w-5xl h-[85vh] bg-[#0a0a0a] border border-white/10 rounded-3xl shadow-2xl flex flex-col overflow-hidden">
                <button className="absolute top-6 right-6 z-[60000] p-2 bg-white/5 rounded-full hover:bg-red-500/20 hover:text-red-500 text-white/50 transition-all cursor-pointer" onClick={(e) => { e.stopPropagation(); sound?.playClick(); onClose(); }}>
                    <X size={20} />
                </button>
                
                <div className="flex justify-between items-end p-8 border-b border-white/5 bg-[#0a0a0a]">
                    <div>
                        <h2 className="text-3xl font-light text-white tracking-widest uppercase mb-1">{t.vault.title}</h2>
                        <p className="text-[10px] font-mono text-white/40">{favorites.length} {t.vault.items} // PORTFOLIO</p>
                    </div>
                    <div className="text-right mr-12">
                        <span className="block text-[10px] text-white/40 font-mono tracking-wider uppercase">{t.vault.totalValue}</span>
                        <span className="block text-4xl font-light tracking-tighter" style={{color: CORPORATE_BLUE}}>{formatMoney(totalValue)}</span>
                    </div>
                </div>

                <div className="flex-grow overflow-y-auto p-8 bg-[#0a0a0a] custom-scrollbar">
                    {favorites.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-white/20 gap-4">
                            <Layers size={64} strokeWidth={0.5} />
                            <span className="font-mono text-xs tracking-[0.3em] uppercase">{t.vault.empty}</span>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                            {favorites.map((fav) => (
                                <div key={fav.id} className="group relative bg-[#111] border border-white/5 rounded-xl overflow-hidden hover:border-white/20 transition-all duration-300 hover:shadow-lg flex flex-col" onClick={() => { sound?.playDeploy(); onFlyTo(fav); onClose(); }}>
                                    <div className="relative h-48 overflow-hidden cursor-pointer">
                                        <img src={fav.photoUrl} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" onError={(e) => {e.target.src = LUXURY_IMAGES[0]}} />
                                        <div className="absolute top-2 right-2 bg-black/70 backdrop-blur px-2 py-1 rounded border border-white/10">
                                            <span className="text-[10px] text-white font-mono">{fav.precio}</span>
                                        </div>
                                    </div>
                                    <div className="p-4 flex flex-col gap-2 flex-grow justify-between">
                                        <div>
                                            ...
                                            <p className="text-[10px] text-white/40 font-mono mt-1">{fav.id}</p>
                                        </div>
                                        <div className="flex items-center justify-between mt-3 pt-3 border-t border-white/5">
                                            <span className="text-[10px] font-bold tracking-widest hover:underline cursor-pointer" style={{color: CORPORATE_BLUE}}>{t.vault.view}</span>
                                            <button className="text-white/20 hover:text-red-500 transition-colors" onClick={(e) => { e.stopPropagation(); sound?.playClick(); removeFromFavs(fav); }}>
                                                <X size={14} />
                                            </button>
                                        </div>
                                    </div>
...
export const ChatPanel = ({ t, sound, onClose, context }) => {
    ...
    useEffect(() => {
        if (context) { 
            setMsgs([{id: 0, role: 'agent', text: `Hola, veo que le interesa ${context.title}. He preparado el dossier de la propiedad ${context.id}. ¿Desea agendar una visita privada?`}]); 
        } else { 
            setMsgs([{id: 0, role: 'agent', text: t.chat.placeholder}]); 
        }
    }, [context]);
    ...
    const handleSend = () => { 
        ...
        setTimeout(() => { 
            sound?.playPing(); 
            setMsgs(prev => [...prev, {id: Date.now()+1, role: 'agent', text: "Mensaje recibido. Un agente le contactará."}]); 
        }, 1000); 
    };
        ...
        <div className="fixed bottom-28 right-8 w-80 bg-[#0a0a0a]/95 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden shadow-2xl z-[10001] animate-fade-in-up">
            <div className="p-4 border-b border-white/10 flex justify-between items-center bg-[#111]">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-white">{t.chat.agent}</span>
                </div>
            ...
            <div className="h-64 overflow-y-auto p-4 flex flex-col gap-3 custom-scrollbar">
                {msgs.map(m => (
                    <div key={m.id} className={`max-w-[85%] p-3 rounded-xl text-xs ${m.role === 'user' ? 'text-white border border-white/20 self-end' : 'bg-white/5 text-white/80 border border-white/5 self-start'}`} style={m.role === 'user' ? {backgroundColor: CORPORATE_BLUE + '20', borderColor: CORPORATE_BLUE} : {}}>
                        {m.text}
                    </div>
                ))}
            </div>
            ...
            <div className="p-3 border-t border-white/10 flex gap-2">
                <input className="flex-1 bg-transparent border-none outline-none text-xs text-white placeholder-white/30" placeholder={t.chat.placeholder} value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()} />
                <button onClick={handleSend} className="hover:text-white" style={{color: CORPORATE_BLUE}}>
                    <Send size={14}/>
                </button>
            </div>
        </div>
...
export default function UIPanels({ 
  ...
  useEffect(() => {
      if (systemMode === 'EXPLORER') {
          setTimeout(() => {
              addNotification("SYSTEM", "Conexión satelital establecida. Red de agentes activa.", "CHAT");
          }, 1500);
      }
  }, [systemMode]);

  // Si la puerta no está desbloqueada, mostramos Gatekeeper
  if (!gateUnlocked) { 
      return <Gatekeeper onUnlock={() => setGateUnlocked(true)} t={t} sound={sound} />; 
  }

  return (
    <div className="pointer-events-none fixed inset-0 z-50 flex flex-col justify-between text-sans">
       
       {/* 1. TOP BAR (Siempre visible tras desbloqueo) */}
       <TopBar t={t} onGPS={() => { sound?.playClick(); onFlyTo({lat: 40.4168, lng: -3.7038}); }} />
       
       {/* 2. CAPA DE MODOS (Router Visual) */}
       
       {/* MODO A: SELECCIÓN DE ROL */}
       {systemMode === 'GATEWAY' && (
       ...
       {systemMode === 'EXPLORER' && (
         <>
            {/* HUD Derecho: Status & Notificaciones */}
            <StatusDeck 
            ...
            {/* HUD Izquierdo: Controles de Vista 3D/2D */}
            <ViewControlDock 
                onViewChange={() => {}} 
                currentView={{is3D: true, mode: 'dawn'}} 
                t={t} 
                sound={sound} 
            />
            
            {/* Dock Inferior Central: Búsqueda y Navegación */}
            <OmniSearchDock 
                onSearch={onSearch} 
                activeTab={activeTab} 
                setActiveTab={(tab) => {
                    // Si clicamos el mismo tab, lo cerramos volviendo a 'map'
            ...
            {/* --- PANELES FLOTANTES (Condicionados por Estado) --- */}
            ...
            {/* Panel de Detalles de Propiedad (Se abre al seleccionar una propiedad en el mapa) */}
            {selectedProperty && (
                <CommandCenterPanel 
                    ...
                    onContactAgent={() => {
                        sound?.playDeploy();
                        setActiveTab('chat');
                        // Aquí podríamos pasar el contexto al chat automáticamente
                    }} 
                />
            )}
         </>
       ...
       {/* BOTÓN DE RETORNO A GATEWAY (Visible si no estamos en Gateway) */}
       {systemMode !== 'GATEWAY' && (
         <div className="fixed bottom-4 left-4 pointer-events-auto z-[60000] animate-fade-in">
            <button 
                onClick={() => { sound?.playClick(); setSystemMode('GATEWAY'); }} 
                className="px-4 py-2 bg-black/80 backdrop-blur border border-white/10 rounded-full text-[9px] text-white/50 hover:text-white uppercase tracking-widest flex items-center gap-2 hover:border-white/30 transition-all"
            >
