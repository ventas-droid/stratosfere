...
import { CORPORATE_BLUE, NEON_GLOW, TEXT_COLOR } from '../app/components/alive-map/data';
import { getPropertyTier, TIER_CONFIG } from '../app/components/alive-map/property-tiers'; 
...
// 🎛️ FRAGMENTO 2: SMART FILTERS (PRECISIÓN TÁCTICA)
// ==============================================================
            ...
            <div className="pointer-events-auto w-full max-w-4xl mx-auto px-6">
              <div className="bg-black/80 backdrop-blur-2xl rounded-full shadow-[0_10px_40px_rgba(0,0,0,0.5)] border border-white/10 flex items-center p-2 gap-3 ring-1 ring-white/5 transition-all duration-300 hover:border-white/20">
                ...
                <button 
                  ...
                  className={`h-14 w-14 rounded-full flex items-center justify-center transition-all duration-300 ${activePanel === 'FILTERS' ? 'bg-white text-black scale-110 shadow-[0_0_20px_rgba(255,255,255,0.4)]' : 'bg-zinc-900/50 text-zinc-400 hover:text-white hover:bg-zinc-800'}`}
                >
                  <SlidersHorizontal className="w-6 h-6" />
                </button>

                {/* B. Botón AI (EL CEREBRO) */}
                <button 
                  onClick={toggleAI}
                  className={`h-14 px-6 rounded-full flex items-center gap-3 transition-all duration-300 ${activePanel === 'AI_PANEL' ? 'bg-blue-600 text-white scale-105 shadow-[0_0_30px_rgba(37,99,235,0.6)]' : 'bg-zinc-900/50 text-zinc-400 hover:text-blue-400 hover:bg-blue-900/20 hover:border-blue-500/30 border border-transparent'}`}
                >
                ...
                {/* C. Botón Perfil (Decorativo/Funcional) */}
                <button className="h-14 w-14 rounded-full flex items-center justify-center bg-zinc-900/50 text-zinc-400 hover:text-white hover:bg-zinc-800 transition-all">
