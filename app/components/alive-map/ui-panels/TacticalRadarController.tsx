...
// Importamos el motor de humo
import { runAgencyOSSmoke } from '../agency-os/agencyos.smoke';
// Importamos audio si lo tiene, si no, puede comentar esta línea
import { playSynthSound } from './audio'; 

// Servicios base (TAL CUAL LOS TENÍA USTED EN SU CÓDIGO)
...
export default function TacticalRadarController({ targets = [], onClose }: any) {
  ...
  const [customServices, setCustomServices] = useState<any[]>([]); // Para el botón +
  ...
  // --- 2. EFECTO DE MEMORIA (Persistencia) ---
  useEffect(() => {
    // Al cargar, leemos del disco duro del navegador
  ...
  const handleTrabajar = (target: any) => {
    ...
    // Si ya lo trabajamos antes, recuperamos estado
    const isProcessed = processedIds.includes(String(target.id));
    
    if (isProcessed) {
        setMsgStatus("SENT");
        setActiveTab("COMMS");
        // Simulamos recuperar chat previo
        setChatHistory([
            { sender: 'system', text: 'Propuesta enviada anteriormente.' },
  ...
  const handleAddCustomService = () => {
      ...
      setActiveServices([...activeServices, newId]); // Lo auto-seleccionamos
