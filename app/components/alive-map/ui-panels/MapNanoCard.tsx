...
import { Heart, Bed, Bath, Maximize2, Navigation, Building2, Home, Briefcase, LandPlot, Warehouse, Sun, Crown } from 'lucide-react';
...
export default function MapNanoCard(props: any) {
  ...
  useEffect(() => {
    const handleUpdate = (e: any) => {
      ...
      if (String(e.detail.id) === String(id) && e.detail.updates) {
        ...
        if (u.price !== undefined || u.rawPrice !== undefined || u.priceValue !== undefined) setCurrentPrice(safeParsePrice(u.rawPrice ?? u.priceValue, u.price));
        if (u.isPremium !== undefined || u.promotedTier !== undefined || u.isPromoted !== undefined) setIsPremium(u.isPremium === true || u.promotedTier === 'PREMIUM' || u.isPromoted === true);
        if (u.isFire !== undefined) setIsFire(u.isFire === true);
      }
    };
    if (typeof window !== "undefined") window.addEventListener("update-property-signal", handleUpdate as EventListener);
    return () => { if (typeof window !== "undefined") window.removeEventListener("update-property-signal", handleUpdate as EventListener); };
  }, [id]);
  ...
  useEffect(() => {
    ...
    const onSync = (e: any) => { if (String(e?.detail?.id) === String(id)) setLiked(!!e.detail.isFav); };
  ...
  const handleAction = (e: React.MouseEvent, action: string) => {
    ...
    // 🔥 EL CÓDIGO RESTAURADO: LA MALETA COMPLETA CON AVATAR, COVER, WEB Y TELÉFONOS
    ...
    if (action === "open") {
      ...
      if (normalizedCoords) setTimeout(() => { window.dispatchEvent(new CustomEvent('fly-to-location', { detail: { center: normalizedCoords, zoom: 18.5, pitch: 60, duration: 1500 } })); }, 150);
    }
  };
  ...
  return (
    // 🔥 CONTENEDOR LIMPIO, SOLO TARJETA FLOTANTE
    <div className={`pointer-events-auto flex flex-col items-center group relative ${isFire ? 'z-[250]' : 'z-[100]'}`} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
      <div 
        className={`origin-bottom duration-300 ease-out transform transition-[opacity,transform] pb-3
        ${(isHovered || props.forceOpen) ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 translate-y-4 pointer-events-none'}`} 
        onClick={(e) => handleAction(e, 'open')}
      >   
        <div className={`flex rounded-[16px] overflow-hidden cursor-pointer transition-all duration-300 backdrop-blur-xl ${isFire ? 'w-[370px] min-h-[135px] bg-white border-[2px] border-red-500 shadow-[0_0_30px_rgba(239,68,68,0.5)]' : 'w-[270px] min-h-[105px] bg-white/95 border border-slate-200 shadow-2xl'}`}>
            
            <div className={`relative shrink-0 overflow-hidden group/img ${isFire ? 'w-[185px]' : 'w-[135px]'}`}>
                <Image src={img} alt="Propiedad" fill sizes="(max-width: 768px) 100vw, 200px" className="object-cover transition-transform duration-1000 group-hover/img:scale-110" loading="lazy" quality={isFire ? 80 : 60}/>                   
                ...
                <div className="absolute top-2 left-2 flex flex-col gap-1.5">
                    {isFire ? ( <div className="px-1.5 py-0.5 rounded backdrop-blur-md bg-white/95 shadow-sm flex items-center gap-1 border border-red-100 w-fit"><Crown size={10} className="text-red-600 fill-red-500 animate-[pulse_1.5s_ease-in-out_infinite]"/><span className="text-[8px] font-black uppercase tracking-widest text-red-700 leading-none mt-[1px]">FUEGO</span></div> ) : ( <div className="px-1.5 py-0.5 rounded backdrop-blur-md bg-white/90 shadow-sm flex items-center gap-1 border border-white/50 w-fit">{getPropertyIcon(type)}<span className="text-[7px] font-bold uppercase tracking-widest text-gray-800 leading-none mt-[1px]">{type}</span></div> )}
                </div>

                <div className="absolute bottom-2 left-2 z-20">
                    <button onClick={(e) => handleAction(e, 'fav')} className={`flex items-center justify-center rounded-full backdrop-blur-xl border transition-all duration-300 hover:scale-110 active:scale-95 shadow-md ${isFire ? 'w-7 h-7' : 'w-6 h-6'} ${liked ? 'bg-red-500 text-white border-red-400' : 'bg-black/40 text-white border-white/30 hover:bg-black/60'}`}><Heart size={isFire ? 12 : 10} className={`transition-transform duration-300 ${liked ? "fill-current scale-105" : "scale-100"}`} /></button>
                </div>
            </div>
            ...
            <div className={`flex flex-col justify-between p-2.5 flex-1 min-w-0 ${isFire ? 'bg-gradient-to-br from-[#FFF8F6] to-white p-3' : ''}`}>
                <div>
                    <div className="flex justify-between items-start gap-1 mb-0.5">
                        <span className={`font-black uppercase tracking-widest block leading-none truncate ${isFire ? 'text-[7.5px] text-red-500' : 'text-[6.5px] text-slate-400'}`}>{liveData.refCode || "REF"}</span>
                    </div>
                    <h3 className={`font-black text-slate-900 truncate tracking-tight ${isFire ? 'text-[15px] mb-1' : 'text-[12px] mb-0.5'}`}>{liveData?.title || type}</h3>
                    <div className={`${isFire ? 'mb-1.5' : 'mb-1'}`}><span className={`block font-black tracking-tighter leading-none ${isFire ? 'text-[24px] bg-gradient-to-r from-red-600 via-orange-500 to-amber-500 bg-clip-text text-transparent' : 'text-[17px] text-slate-900'}`}>{displayLabel}</span></div>
                    <div className="flex items-start gap-1"><Navigation size={isFire ? 9 : 8} className={`shrink-0 mt-[1px] ${isFire ? 'text-red-400' : 'text-slate-400'}`} /><span className={`font-bold uppercase line-clamp-2 leading-snug ${isFire ? 'tracking-wider text-[8px] text-slate-600' : 'tracking-wide text-[7px] text-slate-500'}`}>{locationText}</span></div>
                </div>

                <div className={`mt-1.5 pt-1.5 flex items-center justify-between border-t ${isFire ? 'border-red-100' : 'border-slate-100'}`}>
                    {showOnlyM2 ? (
                        <div className="flex items-center gap-1 text-slate-800"><Maximize2 size={isFire ? 11 : 9} className={isFire ? "text-red-400" : "text-slate-400"} /><span className={`font-black ${isFire ? 'text-[11px]' : 'text-[9.5px]'}`}>{mBuilt} m²</span></div>
                    ) : (
                        <>
                            <div className="flex items-center gap-1 text-slate-800"><Bed size={isFire ? 11 : 9} className={isFire ? "text-red-400" : "text-slate-400"} /><span className={`font-black ${isFire ? 'text-[11px]' : 'text-[9.5px]'}`}>{rooms}</span></div>
                            <div className="flex items-center gap-1 text-slate-800"><Bath size={isFire ? 11 : 9} className={isFire ? "text-red-400" : "text-slate-400"} /><span className={`font-black ${isFire ? 'text-[11px]' : 'text-[9.5px]'}`}>{baths}</span></div>
                            <div className="flex items-center gap-1 text-slate-800"><Maximize2 size={isFire ? 11 : 9} className={isFire ? "text-red-400" : "text-slate-400"} /><span className={`font-black ${isFire ? 'text-[11px]' : 'text-[9.5px]'}`}>{mBuilt}<span className={`font-bold text-slate-400 ml-[1px] ${isFire ? 'text-[7px]' : 'text-[6px]'}`}>m²</span></span></div>
                        </>
                    )}
                </div>
            </div>
